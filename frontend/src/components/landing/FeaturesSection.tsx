...
const features: Feature[] = [
  {
    icon: (
      <svg viewBox="0 0 24 24" className="w-[26px] h-[26px] stroke-moss fill-none" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round">
...
function FeatureCard({ icon, title, desc }: Feature) {
  return (
    <div className="bg-stem-bg p-9 transition-colors duration-200 hover:bg-[#f0ead8]">
      <div className="mb-5">{icon}</div>
      <h3 className="font-serif text-[1.1rem] font-semibold text-ink mb-2">{title}</h3>
      <p className="font-sans text-[0.875rem] leading-[1.7] text-soil-light">{desc}</p>
    </div>
  );
}
...
export default function FeaturesSection() {
  return (
    <section className="py-28 px-8 max-w-[1120px] mx-auto">
      ...
      <h2 className="font-serif text-[clamp(1.9rem,4vw,2.9rem)] font-bold leading-[1.1] tracking-[-0.02em] text-ink mb-16 max-w-[520px]">
        Built for how teams <em className="italic text-moss">actually think.</em>
      </h2>

      {/* 3-col grid with 1px gap borders via background trick */}
      <div
        className="grid grid-cols-3 max-md:grid-cols-2 max-sm:grid-cols-1 rounded-[6px] overflow-hidden border border-[rgba(74,90,58,0.28)]"
        style={{ background: 'rgba(74,90,58,0.18)', gap: '1px' }}
      >
        {features.map((f) => (
